...
$(function() {
    ...
    window.last_action_popup = false;
    $(document).on("kma.callbackOperator", function(event) {
        window.last_action_popup = "click"
    });
    $(document).on("kma.mouseLeave", function(event) {
        window.last_action_popup = "leave"
    });
    $("form").on("submit", function(event) {
        var form = $(this);
        if (form.closest("#kmacb-form").length || form.closest(".mfp-content").length || form.closest(".puForm").length || form.closest(".modal").length) {
            form.find("input[name=frompopup]").val("1");
            switch (window.last_action_popup) {
                case "click":
                    form.find("input[name=order_source]").val(source_popup_operator);
                    break;
                case "leave":
                    form.find("input[name=order_source]").val(source_popup_out);
                    break
            }
        }
    })
